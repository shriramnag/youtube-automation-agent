# Shiv AI — YouTube Automation Agent
...
[![CI](https://github.com/shriramnag/youtube-automation-agent/actions/workflows/ci.yml/badge.svg)](https://github.com/shriramnag/youtube-automation-agent/actions/workflows/ci.yml)
...
The latest production fixes are included in [`124a1ed`](https://github.com/shriramnag/youtube-automation-agent/commit/124a1ed18e474878d5829812dd632e579adbd860) and [`d52b40d`](https://github.com/shriramnag/youtube-automation-agent/commit/d52b40d94a007caf5e7f256378c0265ddcf09d2b):

- **DarkzSEO works without Python:** the bundled advisory audit is now the default. Shiv AI no longer auto-selects sibling Python checkouts, and a missing or broken explicitly configured external runtime falls back to the bundled audit instead of reporting `No module named darkzseo`.
...
**Shiv AI now has a discoverability adapter layer.** v2.10.0 connects the production pipeline to DarkzSEO without merging the projects or weakening human review, then adds the evidence needed to prove what packaging and strategy actually work:
...
The content preflight works out of the box. Set `DARKZSEO_PATH` only when developing against a separate DarkzSEO 1.4+ checkout. If that optional external runtime is missing or broken, Shiv AI automatically uses the bundled audit.
...
```bash
git clone https://github.com/shriramnag/youtube-automation-agent.git
...
AI video verification has its own **Include paid video probe** checkbox. When enabled, Shiv AI creates the provider's shortest supported test clip, records the external task and model, downloads and decodes the MP4, then removes the temporary asset. It never silently tries a second paid provider.
...
Open **Shorts Repurposing Studio** inside Review Studio and choose **Create 3 Short drafts**. Shiv AI selects self-contained windows from the durable scene timeline and preserves the exact source-scene IDs, start time, duration, rationale, title, description, tags, layout, and inherited review evidence for each candidate. Draft selection is local and does not call an AI provider.
...
The optional external adapter uses a shell-free Python child process, sends content JSON over stdin, and reads JSON-only stdout. The public PyPI `darkzseo` 1.3.3 package has a different site-audit CLI and is not used by Shiv AI.
...
Gemini offers free access for supported text and TTS usage. Gemini AI image generation currently requires paid-tier access; without an image provider, Shiv AI can assemble gradient-based visuals instead.
...
After publication, Shiv AI captures comparable 24-hour and 7-day performance snapshots. It evaluates CTR, retention, engagement, watch time, format, length, hook style, and title style against the channel's own history—not a universal view-count target.
...
When an approved learning calls for better packaging, Shiv AI prepares a control plus title and thumbnail variants for new videos. Review Studio shows those options before approval; the selected combination is the only one handed to the publishing queue. Shiv AI does not silently swap live YouTube metadata.
...
| Stage | What Shiv AI does | What you control |
| --- | --- | --- |
...
Shiv AI distinguishes real MP4 output from simulated placeholders. Simulated output cannot enter the approval or publishing path.
...
Long-form productions use hybrid assembly: Shiv AI generates bounded provider clips for the hook and important sections, fills the remaining timeline locally, mixes the existing narration, and keeps the generated caption file alongside the production. As soon as a provider returns its task ID, Shiv AI persists it before polling so interrupted jobs can resume that known task instead of submitting it again.
...
4. Save the JSON as `config/credentials.json`; Shiv AI uses its configured loopback redirect URI exactly
...
Anonymous milestone reporting is disabled by default and has no built-in collector. It activates only when you explicitly set both telemetry variables. The allowlisted payload contains the milestone name and time, Shiv AI version, OS family, Node major version, and a random installation ID. It never includes credentials, channel data, prompts, topics, titles, filenames, or video contents.
...
[@shriramnag](https://github.com/shriramnag) — Shiv AI, YouTube Automation Agent. Find me on [X](https://x.com/shriramnag).

If Shiv AI saves you time, a star helps it reach more developers.

## Contributing

See [CONTRIBUTING.md](CONTRIBUTING.md) for ground rules (short version: one focused concern per PR, no lockfile churn, lint + tests must pass). For questions and setup help, use [Discussions](https://github.com/shriramnag/youtube-automation-agent/discussions) — Issues is for bugs.
