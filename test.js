  ...
  async testDiscoverabilityPreflight() {
    ...
    try {
      const bundledReport = await new DarkzSEOAdapter({ scriptPath: null }).audit({
        id: 'bundled-audit', platform: 'youtube', brand: 'Shiv AI', title: 'Best workflow review',
      ...
      const production = {
        id: productionId,
        script: { title: 'Shiv AI discoverability', fullScript: 'Detailed content '.repeat(200), sections: [] },
        seo: { title: 'Shiv AI discoverability', description: 'A detailed discoverability review.', chapters: [] },
        provenance: { sources: [] }
      };
      await db.saveProductionSnapshot(production);
      const service = new DiscoverabilityService(db, { adapter: fakeAdapter });
      const first = await service.auditProduction(production, { channel_name: 'Shiv AI' });
      ...
      const second = await service.auditProduction(production, { channel_name: 'Shiv AI' });
      ...
      const unavailable = await unavailableService.auditProduction(production, { channel_name: 'Shiv AI' });
